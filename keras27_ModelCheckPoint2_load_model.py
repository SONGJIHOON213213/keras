...
es = EarlyStopping(monitor='val_loss', patience= 10, mode= 'min', verbose=1, restore_best_weight = True)
mcp = ModelCheckpoint(monitor='val_loss', mode='auto',verbose=1,save_best_only=True, filepath='./_save/MCP/keras27_ModelCheckPoint.hdf5')
#땡기고싶으면 쉬프트탭 다시 오른쪽으로 보내고싶으면 탭탭탭          
model.fit(x_train,y_train, epochs=10, callbacks=[es, mcp,],validation_split=0.2)#호출하겟다
#리스트 2개이상은
'''
