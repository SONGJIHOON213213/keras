...
print(x_train.shape)
print(x_test.shape)
x_train = np.reshape(x_train,(-1,54,1))
x_test = np.reshape(x_test,(-1,54,1))
# (105, 4)
# (45, 4) 
print(x_train.shape)
print(x_test.shape) 
...
# # # #2. 모델구성
model = Sequential()
model.add(Conv1D(5,2,activation='relu', input_shape = (54,1)))
model.add(Flatten())
model.add(Dense(16))
model.add(Dense(16))
model.add(Dense(16))
...
# #4.평가,예측
eva=model.evaluate(x_test,y_test) #eva를 쓰면 acc,loss,가 안에 들어간다.LSTM은연결할려면 리턴시퀀스해야됨 
